...
(cd "$ROOT_DIR/server" && ./zappy -p 8674 -x 10 -y 10 -n team1 -c 20 -f 10 > "$ROOT_DIR/logs/[SERVER][teams=1][clients=15][fork=NO].txt" 2>&1 &)
...
for i in {1..14}; do
    "$ROOT_DIR/client/client" localhost 8674 team1 --no-fork 2> "$ROOT_DIR/logs/[CLIENT-${i}][teams=1][clients=15][fork=NO].txt" &
    sleep 0.5
done
"$ROOT_DIR/client/client" localhost 8674 team1 --no-fork 2> "$ROOT_DIR/logs/[CLIENT-15][teams=1][clients=15][fork=NO].txt" &
